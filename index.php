...
function getData($url) {
    ...
    curl_setopt($ch, CURLOPT_TIMEOUT, 6); curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 3);
...
// AJAX 1: ОБНОВЛЕНИЕ КАРТОЧЕК
if (isset($_GET['get_devices_ajax'])) {
    $devices = getData($api); $dev_status = [];
    ...
    header('Content-Type: application/json'); echo json_encode($dev_status); exit;
}

// AJAX 2: ОБНОВЛЕНИЕ ЛОГОВ (БЕЗ ОШИБОК)
if (isset($_GET['get_logs_ajax'])) {
    ...
    $log_html = "";
    if(empty($logs)) { $log_html = "<tr><td colspan='5' style='text-align:center;'>Нет данных</td></tr>"; } 
    else {
        foreach ($logs as $l) {
            ...
            $log_html .= "<tr><td>".htmlspecialchars($l["id"])."</td><td>".htmlspecialchars($l["device_id"])."</td><td>{$act}</td><td><span class='badge-log {$statusClass}'>{$statusText}</span></td><td>".htmlspecialchars($l["log"] ?? "")."</td></tr>";
        }
    }
    echo $log_html ?: "<tr><td colspan='5' style='text-align:center;'>История пуста</td></tr>"; exit;
}

// ОТПРАВКА КОМАНДЫ (СКРИНШОТ, ВЫКЛЮЧЕНИЕ) В ТАБЛИЦУ
...
<div class="header">
    ...
    <div class="sync-indicator"><span class="dot"></span>Мониторинг</div>
</div>

<div class="container">
    <?php foreach($devices as $d): $dev_id = $d["device_id"]; ?>
    <div class="card" data-id="<?=htmlspecialchars($dev_id)?>">
        <div class="name"><?=htmlspecialchars($d["name"] ?? "ПК")?></div>
        ...
        <div class="row"><b>Активность:</b> <span class="heartbeat-time">...</span></div>
        ...
        <form method="POST" action="index.php" style="margin-bottom:6px;">
            ...
            <input type="hidden" name="device_id" value="<?=htmlspecialchars($dev_id)?>">
            <button type="submit" class="orange" onclick="return confirm('Выключить клиент?')">Закрыть клиента</button>
        </form>

        <form method="POST" action="index.php" style="margin-bottom:6px;">
            <input type="hidden" name="action" value="shutdown">
            <input type="hidden" name="device_id" value="<?=htmlspecialchars($dev_id)?>">
            <button type="submit" class="red" onclick="return confirm('Выключить ПК?')">Выключить ПК</button>
        </form>

        <form method="POST" action="index.php">
            <input type="hidden" name="action" value="delete">
            <input type="hidden" name="device_id" value="<?=htmlspecialchars($dev_id)?>">
            <button type="submit" class="gray-danger" onclick="return confirm('Удалить?')">Удалить из базы</button>
        </form>
    </div>
    <?php endforeach; ?>
</div>
...
<div class="log-section">
    ...
    <table>
        <thead><tr><th>ID Задачи</th><th>ID Устройства</th><th>Действие</th><th>Статус</th><th>Лог</th></tr></thead>
        <tbody id="log-table-body"><tr><td colspan="5" style="text-align:center;">Загрузка...</td></tr></tbody>
    </table>
</div>

<script>
function updateDevices() {
    fetch('?get_devices_ajax=1').then(r => r.json()).then(data => {
        for (let dev_id in data) {
            ...
            if (data[dev_id].has_screenshot) {
                screenBox.innerHTML = `<a href="?download_screen=${encodeURIComponent(dev_id)}" class="btn-link green">📥 Скачать скриншот (${data[dev_id].screenshot_left}с.)</a>`;
            } else {
                screenBox.innerHTML = `<button style="background:#1f2937; color:#4b5563; cursor:not-allowed;" disabled>Нет снимка</button>`;
            }
        }
    }).catch(() => {});
}
function updateLogs() {
    fetch('?get_logs_ajax=1').then(r => r.text()).then(html => {
        document.getElementById('log-table-body').innerHTML = html;
    }).catch(() => {});
}
setInterval(updateDevices, 2500); setInterval(updateLogs, 8000);
updateDevices(); updateLogs();
</script>
</body>
</html>
